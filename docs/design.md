...
- **渐变与图片底上的对比度(第四十二条)**:`palette.spec.ts` 与「按声明色合成」那一条都只
  ...
  没进卡片的淡墨小字(章节副题、栏目标签、列表注脚)掉到 3.8:1 以下。现在云雾按主题给明度
  (`.mist` / `.mist-soft`,夜间 0.08 / 0.06)。
- **两百档字号(第四十一条)**:390 宽的机型按 200% 放大后,布局视口只剩 195 CSS px ——
  比 320 那一档还窄 125px,量的是**缩放档**而不是宽度档。判据与巡页共用同一把尺子:
  横向溢出、外壳被滚偏、越界元素、主值行折行。它抓到两处:设置页的 `input[type=range]`
  按原生最小宽度把整行顶出去 6px(`min-w-0`),修炼页的修为行被挤成两行(`flex-wrap`,
  值整块落到第二行而不是折断)。
- **28px 触达下限的量法确定下来(第四十三条)**:这条门曾经在 1280×800 的 /adventure 上
  报过一次「28px」的红 —— 打印出来四舍五入正好等于下限,实际是 27.6px 的子像素抖动,
  而那个按钮声明的是 `min-h-[28px]`。一条会自己变红的门比没有门更糟,故四处量 28px 的
  地方统一成:声明的 `min-height` 到了 28 的给半像素容差,报数带一位小数。
