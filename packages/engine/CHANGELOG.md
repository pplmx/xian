...
- **长线只加 patch**:0.1.x 一直走下去(0.1.1 → 0.1.2 → … → 0.1.9 → 0.1.12 → 0.1.34),
  像 Rust 生态里 0.1 系列常见的那种用法 —— 起步阶段改动密,但编号不该跟着跳;
  minor 只在真有里程碑时跳(例如 API 稳定到可以做 1.0 的准备)。
...
- 编号对齐说明:`v0.1.2`–`v0.1.5` 四份发布原本被编成 0.2.0–0.5.0(同一天内连跳四次),
  按上面的口径重新对齐过编号 —— 那几条 tag 只存在了几个小时、没有任何使用者。
