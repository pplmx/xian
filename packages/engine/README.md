...
# 或
npm  i github:pplmx/wanxiang-engine#v0.1.17
```
...
| 判据 | 钉住的事 | 在哪 |
| --- | --- | --- |
| 用例 | 公开面的行为,506 个用例 / 68 个文件(零运行时依赖,`bun install && bun run test` 即可跑) | `src/**/*.spec.ts` |
...
- 当前版本 **0.1.17**(tag `v0.1.17`)。尚未发布到 npm,按 tag 引用:见[安装](#安装)。
...
packages/engine/
  ...
  examples/         可跑示例(从零装配 / 快速上手 / 最小循环 / 战斗组合技 / 书桌与日常 / 自习室的一天 / 星屑集册 / 一角点心铺 / 行商十二日 / 战斗接管 / 拳赛接管)
  docs/
