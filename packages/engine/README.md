...
```bash
bun add github:pplmx/wanxiang-engine#v0.1.14
# 或
npm  i github:pplmx/wanxiang-engine#v0.1.14
```
...
| 判据 | 钉住的事 | 在哪 |
| --- | --- | --- |
| 用例 | 公开面的行为,477 个用例 / 61 个文件(零运行时依赖,`bun install && bun run test` 即可跑) | `src/**/*.spec.ts` |
...
- 当前版本 **0.1.14**(tag `v0.1.14`)。尚未发布到 npm,按 tag 引用:见[安装](#安装)。
