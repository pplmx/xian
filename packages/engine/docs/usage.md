...
| **曲线体检**(哪一格跳得最狠 / 玩家什么时候开始碾压内容) | `createProgressionAudit({ realms, power, contentPower, crushRatio })` —— 沿等级阶梯逐格量:需求与面板的**相邻格倍数**、**换界那几格**单列(跳变记在落点上)、玩家 ÷ 内容 ≥ `crushRatio`(默认 3)算"碾压";`lines()` 给一行一格读数,`summary()` 给最狠的两处跳变与第一次碾压 |
