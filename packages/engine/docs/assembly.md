...
与每个示例文件都会被自检核对(库这一侧的 `scripts/verify-dist.mjs`,以及**上游工程**里那份
独立成库自检 —— 它在工程仓库里,不在这个包里),所以它不会烂在文档里:拆了模块、改了名字,
自检当场就红。
...
| **数值涨到多大就该换大数** | `Numeric<T>` 适配器 | `numeric.sim.spec.ts`(安全区到 2^53;面板先失真:同一个"1.0京"盖住 5e14;×1.5 每层到第 81 层、×3.2 到第 28 层;换成 bigint 的实测对照) |
