 ...
 *
 * 从前这里还有第三件事:把第三方统计脚本抛的异常分流掉,免得它把判据染红。
 * **那件事连同统计脚本一起删掉了** —— 页面不加载任何外部脚本、不发任何外部请求
 * (见 index.html 里那段说明与隐私声明第六节)。于是这里反过来成了一条**承诺**:
 * 拦下的条数应当是 **0**(判据在 layout-check 与 ui-smoke 的收尾处)。
 *
 * 留着拦截本身有两个用处:① 它是"零外部请求"这条承诺的量尺;
 * ② 万一将来有人顺手加了外链,门会红 —— 而不是让第三方服务器决定我们的门要不要绿
 * (实测红过一次:统计脚本卡住 → `goto(waitUntil: 'load')` 30 秒超时)。
 */
