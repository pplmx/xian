 ...
 *
 * 每一条写的是**族类**(`family`),图标由 `data/beastFamilies.ts` 的族类表给出 ——
 * 从此不手写 icon:狼写 wolf、蛇蛟写 serpent、傀儡写 puppet。原先这里是直接写 icon,
 * 于是「大类够用就行」把 46 只敌人塞进一枚骷髅(独角妖狼、霞光巨蟒、黑风妖王、玄冰蛟…),
 * 灵宠那边应龙与螭龙幼子是星芒、麒麟是兽爪。族类与形的关系现在只有一处,见
 * `BEAST_FAMILIES`;判据在 `src/ui/inkIcons.spec.ts`。
 */
import type { BeastFamily, BossArchetype, BossPhase, ElementId, EnemyDef, EnemySkill, StatMods } from '@/types'
import { beastIcon } from './beastFamilies'
...
export const ENEMIES: EnemyDef[] = [
  // t1 青云山麓
  e('e_wolf', '赤目野狼', 1, 'wolf', [0.9, 1.0, 0.8, 1.0], [bite()]),
  e('e_boar', '山间野猪', 1, 'boar', [1.2, 0.9, 1.0, 0.85], [heavy('猪突', 1.6, 0.18)]),
  e('e_wolfking', '独角妖狼', 1, 'wolf', [2.8, 1.3, 0.85, 1.15], [{ name: '裂空爪', mult: 1.8, rate: 0.3 }], { boss: true, archetype: 'berserk', phases: [{ hpThreshold: 0.5, modChanges: { damageBonus: 0.5 }, skillChanges: [{ name: '狂化·裂空', mult: 2.2, rate: 0.4 }], label: '狂化' }] }),
  // t2 落霞谷
  e('e_sparrow', '落霞灵雀', 2, 'bird', [0.8, 1.1, 0.7, 1.25], [heavy('霞光啄', 1.5, 0.25)]),
  e('e_stoneape', '谷中石猿', 2, 'ape', [1.3, 1.0, 1.2, 0.9], [heavy('投石', 1.5, 0.2)]),
  ...
  // t3 黑风林
  e('e_bwolf', '黑风狼', 3, 'wolf', [1.0, 1.1, 0.9, 1.15], [bite('黑风爪', 1.5)], { element: 'wind' }),
  e('e_vine', '噬人藤', 3, 'plant', [1.4, 0.9, 1.2, 0.7], [{ name: '缠绕吸血', mult: 1.3, rate: 0.3, effect: 'drain' }], {
    element: 'wood'
  }),
  ...
  e('e_bat', '幽窟蝙蝠', 4, 'bat', [0.85, 1.15, 0.8, 1.3], [{ name: '音波', mult: 1.4, rate: 0.25, effect: 'stun' }], { element: 'dark' }),
  e('e_icejiao', '玄冰蛟', 4, 'serpent', [3.4, 1.5, 1.3, 1.1], [{ name: '冰封吐息', mult: 1.6, rate: 0.35, effect: 'pierce' }], { boss: true, element: 'ice', archetype: 'truedmg', phases: [{ hpThreshold: 0.6, skillChanges: [{ name: '冰封·贯体', mult: 2.3, rate: 0.35, effect: 'pierce' }], label: '冰封贯体' }] }),
  // t5 万妖林
  e('e_fox', '三尾灵狐', 5, 'fox', [0.95, 1.2, 0.85, 1.25], [{ name: '魅惑', mult: 1.5, rate: 0.25, effect: 'stun' }], {
    mods: { dodgeRate: 0.15 }
  }),
  e('e_bear', '铁背妖熊', 5, 'bear', [1.5, 1.05, 1.3, 0.8], [heavy('熊霸一击', 1.8, 0.22)]),
  ...
  // t6 古战场遗迹
  e('e_soldier', '残魂兵俑', 6, 'puppet', [1.1, 1.1, 1.1, 0.95], [heavy('残戈', 1.5, 0.25)], { element: 'dark' }),
  ...
  // t7 赤炎火域
  e('e_firewolf', '赤炎狼', 7, 'wolf', [1.05, 1.25, 0.9, 1.15], [heavy('炎爪', 1.6, 0.28)], { element: 'fire' }),
  e('e_golem', '火岩傀儡', 7, 'puppet', [1.6, 1.0, 1.5, 0.7], [{ name: '熔岩护体', mult: 1.4, rate: 0.25, effect: 'shield' }], {
    element: 'fire'
  }),
  ...
  e('e_seaking', '幽冥海皇', 8, 'demon', [5.0, 1.1, 1.4, 1.0], [{ name: '幽冥潮汐', mult: 1.2, rate: 0.5, effect: 'drain' }, { name: '潮汐回复', mult: 1.0, rate: 0.2, effect: 'drain' }], {
    boss: true,
    element: 'water',
    archetype: 'antiheal',
    mods: { lifesteal: 0.15 },
  ...
  e('e_mud', '沼泽泥怪', 9, 'water', [1.7, 0.95, 1.4, 0.65], [{ name: '泥沼吞噬', mult: 1.5, rate: 0.25, effect: 'stun' }], {
    element: 'earth'
  }),
  ...
  e('e_swordlord', '剑冢之主', 10, 'sword', [4.2, 1.7, 1.4, 1.05], [{ name: '万剑归一', mult: 1.8, rate: 0.35, effect: 'pierce' }], { boss: true, element: 'metal', archetype: 'truedmg', mods: { critDamage: 0.5 }, phases: [{ hpThreshold: 0.6, skillChanges: [{ name: '万剑·贯体', mult: 2.7, rate: 0.35, effect: 'pierce' }], label: '万剑贯体' }] }),
  // t11 雷泽
  e('e_leijiao', '雷泽妖蛟', 11, 'serpent', [1.25, 1.3, 1.05, 1.1], [{ name: '雷吻', mult: 1.8, rate: 0.28, effect: 'stun' }], {
    element: 'thunder'
  }),
  ...
  // t12 昆仑雪岭
  e('e_iceape', '雪岭冰猿', 12, 'ape', [1.4, 1.2, 1.25, 0.9], [heavy('冰锤', 1.8, 0.25)], { element: 'ice' }),
  e('e_frostwolf', '霜狼', 12, 'wolf', [1.1, 1.3, 1.0, 1.2], [bite('霜牙', 1.7, 0.3)], { element: 'ice' }),
  ...
  // t13 荒古妖庭
  e('e_guard', '妖庭卫士', 13, 'guard', [1.3, 1.3, 1.2, 1.0], [heavy('妖庭戟法', 1.9, 0.28)]),
  ...
  e('e_gargoyle', '石像鬼', 14, 'puppet', [1.4, 1.3, 1.4, 0.95], [{ name: '石化凝视', mult: 1.8, rate: 0.25, effect: 'stun' }], {
    element: 'earth'
  }),
  ...
  // t15 蜃楼幻海
  e('e_shen', '蜃妖', 15, 'demon', [1.3, 1.4, 1.1, 1.15], [{ name: '幻境迷心', mult: 1.9, rate: 0.3, effect: 'stun' }], {
    element: 'water'
  }),
  ...
  e('e_shenlord', '蜃楼之主', 15, 'demon', [5.4, 1.6, 1.3, 1.35], [{ name: '海市蜃楼', mult: 1.3, rate: 0.4, effect: 'multi' }], { boss: true, element: 'water', archetype: 'evasive', mods: { dodgeRate: 0.35 }, phases: [{ hpThreshold: 0.5, modChanges: { dodgeRate: 0.55 }, label: '蜃楼幻境' }] }),
  // t16 九幽魔渊
  e('e_devil', '魔渊恶鬼', 16, 'demon', [1.35, 1.45, 1.15, 1.1], [{ name: '恶鬼噬魂', mult: 2.0, rate: 0.3, effect: 'drain' }], {
    element: 'dark'
  }),
  ...
  // t17 星陨荒原
  e('e_starpuppet', '星陨傀儡', 17, 'puppet', [1.5, 1.4, 1.4, 0.9], [{ name: '星辉贯穿炮', mult: 1.6, rate: 0.3, effect: 'pierce' }], {
    element: 'light'
  }),
  ...
  e('e_voidgod', '虚空古神', 18, 'god', [5.8, 1.8, 1.4, 1.1], [{ name: '湮灭之瞳', mult: 2.2, rate: 0.35, effect: 'pierce' }], { boss: true, archetype: 'truedmg', phases: [{ hpThreshold: 0.6, skillChanges: [{ name: '湮灭·贯穿', mult: 3.4, rate: 0.35, effect: 'pierce' }], label: '贯穿' }] }),
  // t19 仙人遗府
  e('e_xiangui', '守府仙傀', 19, 'puppet', [1.6, 1.5, 1.5, 1.0], [{ name: '仙法禁制', mult: 2.3, rate: 0.3, effect: 'shield' }], {
    element: 'light'
  }),
  e('e_shilin', '仙府石麟', 19, 'qilin', [1.8, 1.45, 1.6, 0.9], [heavy('麒麟踏天', 2.4, 0.28)], { element: 'light' }),
  ...
  // ============ 仙界(t21-25)============
  // t21 云海仙门
  e('e_imm_guard', '云海仙卫', 21, 'guard', [1.3, 1.35, 1.25, 1.05], [heavy('仙门戟', 2.0, 0.3)], { element: 'metal' }),
  e('e_imm_crane', '接引仙鹤', 21, 'bird', [1.0, 1.3, 0.95, 1.35], [{ name: '鹤唳长空', mult: 1.1, rate: 0.45, effect: 'multi' }], {
    element: 'wind',
    mods: { dodgeRate: 0.12 }
  }),
  e('e_imm_gate', '仙门执事', 21, 'immortal', [5.0, 1.5, 1.5, 1.1], [{ name: '仙门禁制', mult: 2.6, rate: 0.3, effect: 'shield' }], {
    boss: true,
    element: 'metal',
    archetype: 'threshold',
    mods: { shieldOnStart: 0.3 },
  ...
  // t22 金阙玉京
  e('e_imm_jade', '玉京天兵', 22, 'guard', [1.4, 1.4, 1.3, 1.0], [heavy('金阙戟法', 2.1, 0.3)], { element: 'metal' }),
  e('e_imm_spear', '金阙枪灵', 22, 'spear', [1.1, 1.5, 1.0, 1.25], [{ name: '枪出如龙', mult: 2.0, rate: 0.32, effect: 'pierce' }], {
    element: 'metal'
  }),
  e('e_imm_general', '金阙仙将', 22, 'guard', [5.2, 1.8, 1.4, 1.1], [heavy('仙将镇岳', 2.6, 0.32)], {
    boss: true,
    element: 'metal',
    archetype: 'berserk',
  ...
  e('e_tai_zun', '太乙雷尊', 24, 'god', [5.4, 2.2, 0.95, 1.25], [{ name: '太乙神雷', mult: 2.9, rate: 0.32, effect: 'stun' }], {
    boss: true,
    element: 'thunder',
    archetype: 'berserk',
  ...
  e('e_luo_void', '虚空仙将', 25, 'guard', [1.15, 1.5, 1.05, 1.35], [{ name: '虚空裂斩', mult: 2.3, rate: 0.32 }], {
    element: 'wind',
    mods: { dodgeRate: 0.22 }
  }),
  e('e_luo_lord', '大罗天主', 25, 'god', [6.0, 2.0, 1.5, 1.2], [{ name: '大罗神通', mult: 2.8, rate: 0.32 }], {
    boss: true,
    element: 'light',
    archetype: 'evasive',
    mods: { dodgeRate: 0.3 },
  ...
  // ============ 神界(t26-29)============
  // t26 神域边陲
  e('e_god_scout', '神域斥候', 26, 'guard', [1.2, 1.5, 1.15, 1.35], [heavy('神风探路', 2.3, 0.3)], { element: 'wind' }),
  e('e_god_beast', '神界凶兽', 26, 'beast', [1.8, 1.3, 1.4, 0.95], [{ name: '凶兽撕裂', mult: 2.4, rate: 0.3, effect: 'bleed' }], {
    element: 'earth'
  }),
  e('e_god_border', '边陲神将', 26, 'guard', [5.6, 2.0, 1.45, 1.15], [{ name: '神威镇守', mult: 2.9, rate: 0.32, effect: 'shield' }], {
    boss: true,
    element: 'wind',
    archetype: 'threshold',
  ...
  // t27 神兵天关
  e('e_god_soldier', '神兵卫', 27, 'guard', [1.5, 1.6, 1.5, 1.05], [heavy('神兵重斩', 2.6, 0.3)], { element: 'metal' }),
  e('e_god_chariot', '战车神兽', 27, 'beast', [2.0, 1.35, 1.5, 0.9], [{ name: '神车碾踏', mult: 1.3, rate: 0.5, effect: 'multi' }], {
    element: 'earth'
  }),
  e('e_god_general', '神将统领', 27, 'guard', [6.2, 2.1, 1.5, 1.15], [{ name: '神兵天罚', mult: 3.0, rate: 0.32 }], {
    boss: true,
    element: 'metal',
    archetype: 'berserk',
  ...
  // t28 神王圣殿
  e('e_god_priest', '圣殿神官', 28, 'god', [1.3, 1.7, 1.2, 1.2], [{ name: '神言噬魂', mult: 2.5, rate: 0.3, effect: 'drain' }], {
    element: 'light'
  }),
  e('e_god_light', '圣光神使', 28, 'star', [1.25, 1.6, 1.25, 1.3], [{ name: '圣光贯世', mult: 2.6, rate: 0.32, effect: 'pierce' }], {
    element: 'light'
  }),
  e('e_god_king', '神王', 28, 'god', [6.6, 2.2, 1.6, 1.2], [{ name: '神王法谕', mult: 3.1, rate: 0.32 }], {
    boss: true,
    element: 'light',
    archetype: 'spellbane',
    mods: { damageReduction: 0.2 },
  ...
  // t29 神帝天宫
  e('e_god_guard', '天宫神卫', 29, 'guard', [1.7, 1.7, 1.6, 1.1], [heavy('天宫镇守', 2.9, 0.3)], { element: 'metal' }),
  e('e_god_law', '法则神兽', 29, 'beast', [1.5, 1.8, 1.35, 1.25], [{ name: '法则崩灭', mult: 2.9, rate: 0.32, effect: 'pierce' }], {
    element: 'chaos'
  }),
  e('e_god_emperor', '神帝', 29, 'god', [7.0, 2.4, 1.6, 1.25], [{ name: '神帝一念', mult: 3.3, rate: 0.32, effect: 'pierce' }], {
    boss: true,
    element: 'chaos',
    archetype: 'threshold',
    mods: { critRate: 0.15 },
  ...
  // t21 谪仙古渡
  e('e_banished', '谪仙守卫', 21, 'guard', [1.4, 1.35, 1.3, 1.0], [heavy('谪仙戟', 2.0, 0.3)], { element: 'metal' }),
  e('e_duchuan', '渡厄仙槎', 21, 'water', [1.2, 1.3, 1.1, 1.15], [{ name: '渡厄浪', mult: 1.2, rate: 0.45, effect: 'multi' }], {
    element: 'water'
  }),
  ...
  // t22 星陨仙台
  e('e_meteorguard', '星陨仙傀', 22, 'puppet', [1.35, 1.45, 1.2, 1.05], [heavy('陨星锤', 2.1, 0.3)], { element: 'light' }),
  ...
  e('e_ironbody', '不坏金卫', 23, 'guard', [1.5, 1.2, 1.6, 0.85], [{ name: '不朽壁', mult: 1.8, rate: 0.3, effect: 'shield' }], {
    element: 'metal'
  }),
  e('e_jinyuanzhu', '金渊之主', 23, 'immortal', [5.6, 1.7, 1.7, 1.0], [{ name: '不朽金身', mult: 2.6, rate: 0.3, effect: 'shield' }], {
    boss: true,
    element: 'metal',
    archetype: 'threshold',
  ...
  e('e_puppet', '机关仙傀', 24, 'puppet', [1.6, 1.35, 1.45, 1.0], [{ name: '机括连击', mult: 1.25, rate: 0.5, effect: 'multi' }], {
    element: 'metal'
  }),
  ...
  e('e_meteorimmortal', '陨星仙兽', 25, 'beast', [1.8, 1.4, 1.4, 1.0], [heavy('陨星冲撞', 2.5, 0.3)], { element: 'earth' }),
  e('e_xinghaizhu', '星海之主', 25, 'star', [6.1, 2.0, 1.4, 1.3], [{ name: '星海乱流', mult: 1.3, rate: 0.5, effect: 'multi' }], {
    boss: true,
    element: 'light',
    archetype: 'evasive',
    mods: { dodgeRate: 0.28 },
  ...
  // t26 神迹荒原
  e('e_miraclepuppet', '神迹傀儡', 26, 'puppet', [1.5, 1.5, 1.5, 1.0], [heavy('神迹重击', 2.4, 0.3)]),
  e('e_wildbeast', '荒原神兽', 26, 'beast', [1.8, 1.35, 1.4, 1.0], [{ name: '荒神撕咬', mult: 2.4, rate: 0.3, effect: 'bleed' }], {
    element: 'earth'
  }),
  e('e_miraclekeeper', '神迹守者', 26, 'guard', [5.7, 2.0, 1.5, 1.1], [{ name: '神迹镇压', mult: 2.9, rate: 0.32 }], {
    boss: true,
    archetype: 'spellbane',
  ...
  e('e_battlelord', '战场主宰', 27, 'god', [6.3, 2.2, 1.4, 1.2], [heavy('主宰一击', 3.1, 0.32)], {
    boss: true,
    element: 'dark',
    archetype: 'berserk',
  ...
  // t28 万神殿堂
  e('e_idol', '神像卫', 28, 'puppet', [1.7, 1.6, 1.6, 1.0], [{ name: '神像壁垒', mult: 2.3, rate: 0.3, effect: 'shield' }], {
    element: 'light'
  }),
  e('e_hymn', '颂神使', 28, 'star', [1.2, 1.7, 1.2, 1.3], [{ name: '颂神光', mult: 2.6, rate: 0.32, effect: 'pierce' }], {
    element: 'light'
  }),
  e('e_pantheonlord', '万神殿主', 28, 'god', [6.7, 2.1, 1.6, 1.15], [{ name: '万神加护', mult: 3.0, rate: 0.3, effect: 'shield' }], {
    boss: true,
    element: 'light',
    archetype: 'threshold',
  ...
  // t29 帝阙天阶
  e('e_tianjiewei', '天阶神卫', 29, 'guard', [1.6, 1.7, 1.55, 1.1], [{ name: '阶前反戈', mult: 1.1, rate: 0.55, effect: 'multi' }], {
    element: 'metal'
  }),
  e('e_royalseal', '帝印神兽', 29, 'treasure', [1.5, 1.8, 1.35, 1.2], [{ name: '帝印镇落', mult: 2.9, rate: 0.32, effect: 'pierce' }], {
    element: 'chaos'
  }),
  e('e_tianjiekeeper', '帝阙守者', 29, 'god', [7.1, 2.3, 1.6, 1.2], [{ name: '帝阙噬灵', mult: 3.2, rate: 0.3, effect: 'drain' }], {
    boss: true,
    element: 'chaos',
    archetype: 'antiheal',
  ...
  e('e_jitanlord', '古祭之主', 31, 'demon', [7.5, 2.4, 1.6, 1.2], [heavy('开祭一击', 3.5, 0.32)], {
    boss: true,
    element: 'chaos',
    archetype: 'berserk',
